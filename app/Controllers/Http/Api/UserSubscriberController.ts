...
export default class UserSubscriberController extends RestController
{
    ...
    constructor() {
        ...
        this.__resource = "UserSubscriber";
    ...
    private async storeValidation()
    {
        ...
        try{
          validator = await this.__request.validate({ schema: validationRules,messages: {
                      required: '{{ field }} is required to subscribe',
                      'user_email.unique': 'email already exist in our recorde',
          }
         })
        } catch(error){

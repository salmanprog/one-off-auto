...
const ApplicationProduct = () => {
  ...
  const {  data } = useFetch("get_product");
  // Search
  ...
  const column = [
    ...
    {
      title: 'Category',
      dataIndex: 'product_cate_title',
      key: 'product_cate_title',
    },
    {
      title: 'Price',
      dataIndex: 'amount',
  ...
  const redirect_page = () => {
    return navigate("/admin/product/add");
  }
  return (
    
    <ContentWrapper title='All Product'>
      <section className="main-content" style={{padding: '30px'}}>
        <div className='row'>
          ...
          <Button type="primary" onClick={redirect_page} style={{marginBottom:'10px',float:'right'}} htmlType="submit">Add New</Button>
            <Table columns={column} data={filteredData} actions={['edit','delete']} delete_opt='delete_product'/>

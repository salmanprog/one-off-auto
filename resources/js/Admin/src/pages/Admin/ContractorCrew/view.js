...
const ViewCrew = () => {
  ...
  const redirect_page = (slug) => {
    return navigate("/admin/contractor-job/view/"+slug);
  }
  ...
  if (!window._.isEmpty(userProfile)) {
    userProfile.forEach((value, key) =>
      profiles.push({
        ...
        view_job: <button onClick={() => { redirect_page(value.slug); }} type="button" className="btn btn-sm btn-primary">View Job</button>
      })
    );
  }
...
export default React.memo(ViewCrew);

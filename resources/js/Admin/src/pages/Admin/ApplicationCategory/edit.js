...
const EditCategory = () => {
  let { slug } = useParams();
  const { data } = useFetch("view_application_category", "mount", slug);
  const { data: categories } = useFetch("get_application_category", "mount", '?limit=5000&except_slug=' + slug);
  const { loading, postData } = useFetch("update_application_category", "submit");
  ...
  const onFinish = (values) => {
    ...
    const callback = (receivedData) => {
      return navigate("/admin/application-category/edit/" + slug);
    };
          ...
          <div className="col-sm-12">
            {
              ...
              >
                ...
                <div className='row'>
                  <div className='col-12 col-md-6'>
                    <BaseInput label="Category Type asasd" name="category_type" type="select" defaultValue={data.type} options={cat_type} />
                  </div>
                </div>
                <div className='row'>
                  <div className='col-12 col-md-12'>
                    {
                      treeData.length && data ?
                        <TreeSelect
                          ...
                          treeDefaultExpandedKeys={[treeData.find(i => i.id == data.parent_id)?.pId]}
                          treeData={treeData}
                        />
                        :
                        <></>

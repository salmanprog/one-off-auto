...
const ApplicationUserFeedback = () => {
  ...
  const {  data } = useFetch("get_user_feedback");
  // Search
  ...
  useEffect(() => {
    if (searchText === '') {
      ...
      const filtered = data.filter(item =>
        item.created_name.toLowerCase().includes(searchText.toLowerCase())
      );
  ...
  const redirect_page = () => {
    return navigate("/admin/feedback/add");
  }
  return (
    
    <ContentWrapper title='All UserFeedback'>
      <section className="main-content" style={{padding: '30px'}}>
        <div className='row'>
          ...
          <div className="col-sm-12">
            <Table columns={column} data={filteredData} actions={['view']}/>
...
export default React.memo(ApplicationUserFeedback);
...
const ApplicationProductCategory = () => {
  ...
  const {  data } = useFetch("get_product_category");
  // Search
  ...
  const redirect_page = () => {
    return navigate("/admin/product-category/add");
  }
  return (
      ...
      <section className="main-content" style={{padding: '30px'}}>
        <div className='row'>
          ...
          <Button type="primary" onClick={redirect_page} style={{marginBottom:'10px',float:'right'}} htmlType="submit">Add New</Button>
            <Table columns={column} data={filteredData} actions={['edit','delete']} delete_opt='delete_product_category'/>

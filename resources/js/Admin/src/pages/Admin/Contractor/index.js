...
const ApplicationContractor = () => {
  let navigate = useNavigate();
  let user = window.helpers.getStorageData('session');
  const { data } = useFetch("get_users", "mount", "?parent_id=" + user.id + "&user_group_id=3");
  ...
  const redirect_page = () => {
    return navigate("/admin/contractor/add");
  }
  ...
  const column = [
    ...
    {
      title: 'Company Email',
      dataIndex: 'company_email_address',
  ...
  return (
    <ContentWrapper title='All Contractors'>
      <section className="main-content" style={{ padding: '30px' }}>
        <div className="row">
          ...
          <div className="col-sm-12">
            <Button type="primary" onClick={redirect_page} style={{ float: 'right', marginBottom: '10px' }}>Add New</Button>
...
export default React.memo(ApplicationContractor);
